...
locals() #показывает все локальные перемены
globals() #показывает все глобальные перемены
#LEGB - local, enclosed, global, build - in
'build - in' - #встроенные пространства имен (все встроенные переменные(print. int ...))
# Global - глобальные пространства имен (все переменные внутри файла, которые создали мы)
# Чтобы узнать, что нах-ся в глобальном пространстве, можно использовать функцию globals
